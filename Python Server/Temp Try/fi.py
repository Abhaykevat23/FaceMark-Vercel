...
class_image = cv2.imread('D:\Mini Project\FaceMark\Python Server\My Try\py code\class.jpeg')
...
folder_path = 'D:\Mini Project\FaceMark\Python Server\My Try\class images/'  # Replace with the path to your folder
...
df = pd.DataFrame(face_data, columns=['Name', 'X', 'Y', 'Width', 'Height'])
df.to_excel('face_comparison_data.xlsx', index=False)

# Display the labeled image (optional)
cv2.imshow('Labeled Faces', class_image)
cv2.waitKey(0)
